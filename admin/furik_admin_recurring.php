...
class Recurring_List extends WP_List_Table {
	...
	private function get_donation_details_html( $item ) {
		...
		$related_transactions = $wpdb->get_results(
			$wpdb->prepare(
		...
		<div class="donation-details">
			<h2><?php printf( __( 'Recurring Donation Details: %s', 'furik' ), $item['transaction_id'] ); ?></h2>
			...
			<div class="donation-info">
				...
				<table class="widefat fixed striped">
					...
					<tr>
						...
						<td><?php echo str_replace( $this->row_actions( array() ), '', $this->column_transaction_status( $item ) ); ?></td>
					</tr>
	...
	protected function process_bulk_action() {
		if ( 'delete' === $this->current_action() ) {
			...
			$donation_ids = isset( $_POST['donation_ids'] ) ? $_POST['donation_ids'] : array();
...
class Recurring_List_Plugin {
	...
	public function __construct() {
		add_filter( 'set-screen-option', array( __CLASS__, 'set_screen' ), 11, 3 );
	...
	public function plugin_menu() {
		$hook = add_submenu_page(
			...
			array( $this, 'recurring_list_page' )
		);
