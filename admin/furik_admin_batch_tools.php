...
function furik_batch_tools_scripts( $hook ) {
	...
	wp_register_style(
		'furik-batch-tools',
		plugins_url( '/furik/css/furik-batch-tools.css', __DIR__ )
	);
	...
	wp_register_script(
		'furik-batch-tools',
		plugins_url( '/furik/js/furik-batch-tools.js', __DIR__ ),
		array( 'jquery' ),
		'1.0',
		true
	);
	...
	foreach ( $payments as $payment ) {
		...
		if ( $previous_date ) {
			...
			if ( $days_since_last < $days_threshold ) {
				...
				$result = sprintf( __( 'Last payment was only %1$d days ago (threshold: %2$d days)', 'furik' ), $days_since_last, $days_threshold );
				++$skipped_count;
				$status_class = 'status-skipped';
			} else {
